...
import { ArrowLeft, ArrowRight, CheckCircle, User, Wrench, ShieldCheck, Mail, Loader2 } from 'lucide-react'

const STEPS = [
  { id: 1, label: 'Basic Details', icon: User },
  { id: 2, label: 'Work Details',  icon: Wrench },
...
export default function WorkerRegister() {
  ...
  const validateStage2 = () => {
    if (!form.workerType) { toast.error('Please select your worker type'); return false }
    return true
  }
  ...
  const validateStage3 = () => {
    ...
    if (!form.consent)                                       { toast.error('Please accept the Terms & Conditions to continue'); return false }
    return true
  }
  ...
  const buildPayload = () => ({
    ...
    address: form.address.trim(), state: form.state,
    district: form.district, block: form.block, pincode: form.pincode,
    workerType: form.workerType,
    experience: form.experience || undefined,
    labourCardNumber: form.labourCardNumber || undefined,
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/auth/worker/register', buildPayload())
  ...
  const handleVerify = async () => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/auth/worker/verify-otp', { mobile: form.mobile, otp })
      login(res.data.token, { ...res.data.user, role: 'worker' }, res.data.refreshToken)
      toast.success('Registration verified! Welcome to MoKama.')
  ...
  const Spinner = () => <Loader2 size={14} className="animate-spin text-[#6b6b6b] inline ml-1" />
            ...
            {STEPS.map(s => {
              ...
              return (
                <div key={s.id} className="flex flex-col items-center gap-1.5 z-10">
                  ...
                  <span className={`text-xs font-medium hidden sm:block ${step >= s.id ? 'text-[#a3a3a3]' : 'text-[#3a3a3a]'}`}>
                    {s.label}
                  </span>
                </div>
              )
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input className="input" placeholder="As per Aadhaar" value={form.name} onChange={e => set('name', e.target.value)} />
                </div>
                <div>
                  <label className="label">Father's Name *</label>
                  <input className="input" placeholder="Father's full name" value={form.fatherName} onChange={e => set('fatherName', e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">Gender *</label>
                    <select className="input bg-[#141414]" value={form.gender} onChange={e => set('gender', e.target.value)}>
                  ...
                  <div>
                    ...
                    <input type="date" className="input" max={today} value={form.dob} onChange={e => set('dob', e.target.value)} />
                ...
                <div>
                  ...
                  <textarea className="input min-h-[72px] resize-none" placeholder="House No., Street, Village/Town"
                    value={form.address} onChange={e => set('address', e.target.value)} />
                </div>
                <div>
                  <label className="label">State * {geoLoading.states && <Spinner />}</label>
                  <select className="input bg-[#141414]" value={form.state} onChange={e => set('state', e.target.value)} disabled={geoLoading.states}>
                    <option value="">{geoLoading.states ? 'Loading states...' : 'Select state / UT'}</option>
                    {states.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="label">District * {geoLoading.districts && <Spinner />}</label>
                  <select className="input bg-[#141414]" value={form.district} onChange={e => set('district', e.target.value)} disabled={!form.state || geoLoading.districts}>
                    <option value="">{geoLoading.districts ? 'Loading districts...' : form.state ? 'Select district' : 'Select state first'}</option>
                    {districts.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
                <div>
                  <label className="label">Block * {geoLoading.blocks && <Spinner />}</label>
                  <select className="input bg-[#141414]" value={form.block} onChange={e => set('block', e.target.value)} disabled={!form.district || geoLoading.blocks}>
                    <option value="">{geoLoading.blocks ? 'Loading blocks...' : form.district ? 'Select block' : 'Select district first'}</option>
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select className="input bg-[#141414]" value={form.workerType} onChange={e => set('workerType', e.target.value)}>
                    <option value="">Select your trade</option>
                    {workerTypes.map(w => <option key={w._id} value={w.name}>{w.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="label">Work Experience <span className="text-[#3a3a3a] text-xs">(optional)</span></label>
                  <input type="number" className="input" placeholder="Years of experience" min={0} max={50}
                ...
                <div>
                  ...
                  <input className="input" placeholder="If you have a government labour card"
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <input className="input flex-1" placeholder="10-digit number" maxLength={10}
                      value={form.mobile} onChange={e => set('mobile', e.target.value.replace(/\D/g, ''))} disabled={otpSent} />
                  </div>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <input className="input text-center text-2xl tracking-[0.5em] font-bold"
                        maxLength={6} placeholder="••••••" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ''))} />
                    </div>
