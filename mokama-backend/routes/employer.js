...
router.put('/profile', protect, requireRole('employer'), async (req, res) => {
  try {
    const { address, state, district, block, pincode, labourCardNumber, labourLicenseNumber } = req.body;
    const employer = await Employer.findByIdAndUpdate(
      req.user._id,
      { address, state, district, block, pincode, labourCardNumber, labourLicenseNumber },
      { new: true }
